...
-- create table ... as select 구문: 테이블의 모양(컬럼 이름, 데이터 타입)을 복사
create table my_emp as select * from emp;

select * from my_emp;

-- 테이블의 모양만 복사, 데이터는 복사하지 않는 경우:
create table my_emp2 as select * from emp where 1 ^= 1;
