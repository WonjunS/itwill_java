...
-- 자바 프로그램에서 데이터베이스의 접속해서 SQL(select/insert/update/delete) 문장들을 실행
...
--   created_time: 블로그 작성 시간. timestamp. 기본값(시스템 현재 시간)
...
BLOGS 테이블에 데이터가 insert될 때마다 primary key로 설정된 ID 컬럼에 시퀀스가 자동으로 저장되도록 하기 위해서
*/
