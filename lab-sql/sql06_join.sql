...
-- left outer join 결과와 right outer join 결과를 합집합(union)
...
-- emp, salgrade 테이블 사용, 사번, 이름, 급여, 급여 등급을 검색
-- 사번 오름차순 정렬
...
-- ex3. 직원 이름, 부서 이름, 급여, 급여 등급 검색. 부서 이름 오름차순, 급여 등급 오름차순
...
-- 부서 번호 오름차순
select d.deptno, dname, count(*), min(sal), max(sal)
from emp e, dept d
where e.deptno = d.deptno
...
-- 정렬 순서: (1) 부서 번호, (2) 사번
select d.deptno, dname, e1.empno, e1.ename, e1.mgr, e2.ename, e1.sal, s.grade
from emp e1 
