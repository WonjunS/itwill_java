...
-- 숫자 타입뿐만 아니라 문자열, 날짜 타입들도 대소비교가 가능(<, >, <=, >=, ...)가 가능
...
-- LIKE 검색: 
--   (1) %: 글자수 제한 없음. 
--   (2) underscore(_): underscore 자리에 어떤 글자가 와도 상관 없음
...
-- 직무 첫 문자는 어떤 문자이든 상관 없고, 나머지 문자열은 LERK로 끝나는 직무
select job
from emp
