...
from emp
where job like (select job from emp where ename = 'ALLEN');
...
select ename, sal, nvl(comm, 0) "COMM", (sal * 12) + nvl(comm, 0) "ANNUAL_SAL"
from emp
...
-- 단일행 서브쿼리: 서브쿼리의 결과가 1개 이하의 행인 경우
-- 다중행 서브쿼리: 서브쿼리의 결과가 2개 이상의 행인 경우
-- 다중행 서브쿼리에서는 한 개의 값과 단순 비교를 할 수 없음
-- 다중행 서브쿼리에서는 in, any, all과 같은 키워드를 함께 사용해야 함

-- 각 부서에서 급여를 가장 많이 받는 직원의 모든 정보(사번, 이름, 급여, ...)를 검색
select *
from emp
where (deptno, sal) in (
...
-- 각 부서에서 급여가 가장 작은 직원들의 모든 정보 출력
select *
from emp
where (deptno, sal) in (
